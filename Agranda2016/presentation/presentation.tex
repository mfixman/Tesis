...
\begin{itemize}
	...
	\begin{description}
		...
		\item[Wealthy People] have more than \num{3600}~MXN.
	\end{description}
	\item Acording to our hypothesis, if a person calls mostly users from one of the groups then its part of that group.
\end{itemize}
\end{frame}
...
\begin{itemize}
	...
	\item<2-> \textbf{Majority Voting} same category as the majority of its contacts. 66\%~accuracy.
	\item<3-> \textbf{Bayesian Inference} where we define a probability distribution for one category using a \textbf{Beta~Distribution}.
\end{itemize}
...
\note{%
	...
	\( \beta^j \left( x; \alpha^j, \beta^j \right) \) is the distribution of probabilities that the user \( j \) is wealthy.
...
\begin{itemize}
		...
		\end{itemize}
		\item \( \beta^j \left( x; \alpha^j, \beta^j \right) \) is the distribution of probabilities that the user \( j \) is wealthy.
...
\note{%
	...
	In this case, we define 4 categories depending on a users' wealth.
...
\begin{itemize}
	...
	\begin{itemize}
		\item \( \alpha^j_i \) is the number it has with users to the category \( i \).
	\end{itemize}
