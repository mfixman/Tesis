...
Namely, the \textit{Spearman's rank correlation}, as defined in Section~\ref{subsec:spearman}, is computed to test the statistical dependence of sets of incomes of callers and callees. Applying this formula to the data gives a correlation coefficient of $\mathbf{r_s = 0.474}$.
...
The naïve way of solving this problem would be to assign this probability using only the current data, as in Equation~\ref{eq:naive_p}.
...
The \emph{Probability Density Function} of the prior for a parameter $\gamma$ is presented in Equation~\ref{eq:jeffreys}. It's interesting to note that this is the same PDF than in the one for a \emph{Beta Distribution} with parameters $\alpha = 0.5, \beta = 0.5$.
...
Once this methods defines which $\varpi \in \left\{ \calls, \etime, \sms, \contacts \right\}$ it will use, it chooses $\Theta$ as to maximize the \emph{Area Under the Curve} of the model. While calculating the \emph{Inverse Cumulative Function} for the \emph{Beta Distribution} is particularly slow, it's possible to calculate less datapoints by using a \emph{Statistical Prior}, as defined in Section~\ref{subsec:modelling_users}.
Another probability (still unexplored) is to figure out that the \emph{Area Under the Curve} for a particular $\varpi$ is monotonic increasing until the optimal $\Theta$, and later is monotonic decreasing, as shown in the later Figure~\ref{fig:theta}. If this hypothesis were true, it would be possible to use \emph{Ternary Search} to find the best value of $\Theta$.

After finding a $\varpi$ and $\Theta$, it's possible to find the point $p_v$ at which the \emph{Beta Distribution} $\Betasim_v$ of each user $v \in V$ integrates to $\Theta$. Having this data we can find the $\tau$ that maximises the \emph{Accuracy} of the model by comparing the predicted categories to the data in the \emph{Testing Set}.
...
In the model, shown in Figure~\ref{fig:bayesnetwork}, doesn't directly define a \emph{Beta Distribution} $\Betasim_v \sim \Betadist(\varpi^{\high}_v + 1, \varpi^{\low}_v + 1)$ for every user $v$. Instead, the problem is defined as the \emph{Binomial Distribution} in Equation~\ref{eq:graphicalmodel_binomial} for some probability $\pi_v$.
