...
The final objecive of this thesis is to detect \emph{High Income} users in a dataset by just knowing their calls using the hypothesis, described in Section~\ref{subsec:income_homophily}, that a user will mostly call people from his same cincome category.
...
\begin{equation}
\label{eq:naive_p}
p_v = P \left( v \in H_2 \right) = \frac{c^{\high}_v}{c^{\high}_v + c^{\low}_v}
\end{equation}
...
\label{eq:beta}
\begin{gathered}
p_v \sim \Beta \left( c^{\high}_v + 1, c^{\low}_v + 1 \right) \\
P \left( p_v \leq x \right) = \frac{1}{\Beta \left( c^{\high} + 1, c^{\low} + 1 \right)} \cdot \int_0^x {t^{c^{\high}} {\left( 1 - t \right)}^{c^{\low}} dt}
\end{gathered}
\end{equation}
...
One advantage of this method is that it's possible to evaluate the method by calculating the \emph{Area Under the Curve} without having to specify a particular $tau$, since this follows the necessary pattern defined in Section~\ref{subsec:auc}.
