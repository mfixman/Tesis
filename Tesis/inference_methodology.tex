...
Instead, given the definition of an arbitrary $\Theta \in \left[ 0, 1 \right]$, initially defined as $\Theta = 0.05$, it's possible to use the formula presented in Section~\ref{subsec:beta_ppf} to define a suitable $p_v$, as shown in Equation~\ref{eq:beta_theta_ppf}.

\todo{Show that the result is the same $\left( \forall \ \Theta \in \left( 0, 1 \right) \right)$}
...
\begin{tikzpicture}
	\node[obsDisc] (n) {$n_v$};
	\node[obsDisc, below = 1 of n] (k) {$k_v$};
	\node[latentCont, below = 1 of k] (pi) {$\pi_v$};
	\node[detCont, below = 1 of pi] (p) {$p_v$};
	\node[latentCont, left = 1 of p] (theta) {$\Theta$};
	\node[detDisc, below = 1 of p] (hkappa) {$\hat{\kappa}_v$};
	\node[obsDisc, below = 1 of hkappa] (kappa) {$\kappa_v$};
...
\begin{align*}
	...
	\Theta &\sim \mathcal{U} \left[ 0, 1 \right] \\
