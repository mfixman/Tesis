...
\Cref{tab:comparison} compares the results of these methods with the \emph{Bayesian Algorithm} described in \cref{sec:inference_methodology} along several metrics.
...
In case of a tie (which happens often when using the \emph{Outer Graph}), the category is chosen randomly. This approach can be formalized as in \cref{eq:majority_voting}.
...
\resizebox{!}{.3\textheight}{%
	\framebox{%
		\include{figures/mlrelationships.tikz}
	}
}
...
The amount of features in each level is described in \cref{tab:datasettable}.
...
Having these groups it's possible to define a set of features similar to the one in \cref{subsec:user_data}, where each feature is separated by the category of the neighbor. \Cref{eq:matcatuserdata} represents the composition of the names of the new features.
...
\bottomrule
\end{tabular}
	\caption{Resulting metrics of different methods used in \cref{sec:comparison} tested in the \emph{Outer Graph} $B^{\test}$, which includes all nodes. Unlike the previous table, the \emph{Bayesian Algorithm} described in \cref{sec:results} can't be tested in this dataset. \textbf{Bolded} items represent the highest value for each metric.}
