...
In case of a tie (which happens often when using the \emph{Outer Graph}), the category is chosen randomly. Theis approach can be formalized as in \cref{eq:majority_voting}.
...
Having these groups it's possible to define a set of features similar to the one in \cref{subsec:user_data}, where each feature is separated by the category of the neighbour. \Cref{eq:matcatuserdata} contains the names of the new features.
