...
Using the previous features, we create datasets on different levels.

\begin{enumerate}
	\item[0] \emph{Local Features} as described in Section~\ref{subsec:user_data}.
	\item[0.5] \emph{Classified Local Features} as described in Section~\ref{subsec:categoricaluserdata}, joined with the data on the previous item. There are two separate sets of sampels used for experiments on this level.
	\begin{description}
		\item[Outer Nodes] which contain all samples.
		\item[Innter Nodes] which only contain samples in the training set $T$ which have at least a neighbour in $T$.
	\end{description}
