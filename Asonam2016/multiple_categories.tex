...
To this end, we separate the income values into five distinct groups $ H_1, \ldots, H_5 \subseteq G$ of increasing wealth. An user is part of this group if its income is between the defined bounds, that is, \( g \in H_i \iff g_s \in R_i \).
...
\caption{ROC curves for multiclass problem. The performances observed are: $\AUC_1 = 0.68$, $\AUC_2 = 0.69$, $\AUC_3 = 0.63$, $\AUC_4 = 0.68$, $\AUC_5 = 0.69$. These predictors performs better than the random case, and have a similar performance (with exception of category 3).}
