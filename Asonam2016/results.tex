...
We examine the true positive ($\TPR$) and false positive ($\FPR$) rates, \( \TPR = \TP / \operatorname{P} \) and \( \FPR = \FP / \operatorname{N} \), where $\TP$ is the number of correctly predicted users with high income, $P$ is the total number of users with high income, $\FP$ is the number of users incorrectly classified as having high income, and $N$ is the total number of users with low income.
...
We can define the accuracy of the model as the ratio between \(\TP\) and \(\TN\) to total population for a certain \(\FPR\), which gives us a number between \num{0.42} and \num{0.71}. The maximum is achieved with an \(\FPR\) of \num{0.4}.
...
The accuracy of the first method is a predictable \num{0.5}, while one for majority voting is \num{0.66}. These are noticeably worse than the \num{0.71} accuracy calculated for the Bayesian method.
